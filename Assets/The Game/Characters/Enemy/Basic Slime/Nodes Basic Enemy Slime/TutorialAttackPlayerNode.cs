...
{
    public class TutorialAttackPlayerNode : IBaseNode
    {
        ...
        public TutorialAttackPlayerNode(NavMeshAgent agent, float attackRange, float offsetDistance, LayerMask attackLayer, float coneWidth, float coneLength, Animator animator, int animIDAnticipate, int animIDAttack)
        {
            this.agent = agent;
        ...
        private bool IsPlayerWithinCone(Vector3 directionToPlayer)
        {
            ...
            if (angleToPlayer <= coneWidth / 2f && directionToPlayer.magnitude <= coneLength)
            {
                ...
                {
                    agent.speed = TutorialAttackSlime.originalSpeed; // Reset agent speed if not attacking
                }
